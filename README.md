...
Spreadsheet formulas are preceded by a comma and use stack-based tacit programming with the preceding row values used as stack values.
...
The input is piped through `teb` to yield formatted output with formula results calculated before the commas.
...
Formulas are written after the commas.
Formulas use reverse Polish notations and take values on the row they're on as stack arguments.
An empty comma will reuse the last formula seen on the same column.
