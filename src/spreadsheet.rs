...
impl Spreadsheet {
    ...
    pub fn eval(&mut self) -> Result<()> {
        for (i, j) in self.posns().collect::<Vec<_>>() {
            ...
            let value = self.eval_at(i, j, &formula)?;
            self.cells[i][j].set_output(value);
        }
        Ok(())
    }

    fn eval_at(&self, i: usize, j: usize, formula: &str) -> Result<Option<Array>> {
        match &self.cells[i][j] {
            Cell::Empty => Ok(None),
            Cell::Input(input) => Ok(Some(input.clone())),
            Cell::Output(_, formula) => {
                todo!("Use VM eval");
            }
        }
    }
    ...
    fn posns(&self) -> impl Iterator<Item = (usize, usize)> {
        debug_assert!(
            self.cells.iter().all(|row| row.len() == self.cells[0].len()),
